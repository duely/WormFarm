...
public class NBTConverter
{
		...
		for(Entry<String,JsonElement> entry : jObj.entrySet())
		{
			...
			} else
			{
				...
				} catch(Exception e)
				{
					if(tags.hasKey(key))
					{
						WormFarm.LOG.info("JSON/NBT formatting conflict on key '" + key + "'. Skipping...");
						continue;
					}
				}
	...
	private static NBTBase JSONtoNBT_Element(JsonElement jObj, byte id, boolean format)
	{
		...
		{
			WormFarm.LOG.info("An error occured while parsing JsonElement to NBTBase (" + tagID + "):", e);
		}

		WormFarm.LOG.info("Unknown NBT representation for " + jObj.toString() + " (ID: " + tagID + ")");
		return new NBTTagString();
	}
